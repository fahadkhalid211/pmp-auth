...
/**
 * Standalone wp-login.php 2FA page.
 * Variables: $method, $masked, $show_tabs, $remember, $expiry, $otp_len, $trust_days.
 *
 * @package PMP2FA_Auth
 */
...
<title><?php echo esc_html( $site ); ?> &#8212; <?php esc_html_e( 'Verify Your Identity', 'pmp-2fa-authentication' ); ?></title>
<?php wp_head(); ?>
</head>
<body class="login pmp2fa-standalone-body">

<div class="pmp2fa-standalone-wrap">

  <div class="pmp2fa-standalone-logo">
    <?php if ( has_custom_logo() ) { the_custom_logo(); } else { echo '<span style="font-size:48px">🔐</span>'; } ?>
  </div>

  <div class="pmp2fa-card pmp2fa-standalone-card">

    <div class="pmp2fa-head">
      <div class="pmp2fa-icon" aria-hidden="true">🔐</div>
      <h2 class="pmp2fa-title"><?php esc_html_e( 'Two-Step Verification', 'pmp-2fa-authentication' ); ?></h2>
      <p class="pmp2fa-sub">
    ...
    <div class="pmp2fa-tabs" role="tablist">
      <button type="button" class="pmp2fa-tab <?php echo ( 'email' === $method ) ? 'active' : ''; ?>" data-method="email" role="tab">&#9993; <?php esc_html_e( 'Email', 'pmp-2fa-authentication' ); ?></button>
      <button type="button" class="pmp2fa-tab <?php echo ( 'sms' === $method ) ? 'active' : ''; ?>" data-method="sms" role="tab">&#128241; <?php esc_html_e( 'SMS', 'pmp-2fa-authentication' ); ?></button>
    </div>
    <?php endif; ?>
    ...
    <form id="pmp2fa-form" novalidate autocomplete="off">
      ...
      <div class="pmp2fa-field">
        ...
        <input type="text" id="pmp2fa-input" class="pmp2fa-input" inputmode="numeric"
          autocomplete="one-time-code" maxlength="<?php echo esc_attr( $otp_len ); ?>"
          placeholder="<?php echo esc_attr( $ph ); ?>" autofocus required>
        <p class="pmp2fa-hint">
