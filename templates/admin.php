 ...
 *
 * @var array $s Current settings (from pmp2fa_get_settings()).
...
<div class="wrap pmp2fa-admin">
	...
	<div class="pmp2fa-admin-header">
		<span class="pmp2fa-admin-icon">🔐</span>
		<div>
	...
	<div class="pmp2fa-admin-layout">
			...
			<form method="post" action="options.php">
					...
					<table class="form-table">
						...
						<tr>
							...
							<td>
								...
								<p class="description"><?php esc_html_e( 'Between 4 and 8 digits.', 'pmp-2fa-authentication' ); ?></p>
							</td>
						</tr>
						<tr>
				...
				<div class="pmp2fa-box" id="pmp2fa-sms-box">
					...
					<div class="notice notice-warning inline" style="margin:0 0 14px;"><p>
						...
						<?php esc_html_e( 'Enter your Account SID, Auth Token, and From Number, then save. Until configured, all OTPs will be sent via email.', 'pmp-2fa-authentication' ); ?>
					</p></div>
					<?php endif; ?>
					...
					<p class="description" style="margin-bottom:14px;">
						...
						printf(
							wp_kses(
								...
								__( 'Requires a <a href="%s" target="_blank" rel="noopener noreferrer">Twilio</a> account. Users must save a phone number in their profile (E.164 format).', 'pmp-2fa-authentication' ),
					...
					<table class="form-table">
						<tr>
							...
							<td><input type="text" name="pmp2fa_settings[twilio_sid]" id="pmp2fa-twilio-sid" value="<?php echo esc_attr( $s['twilio_sid'] ); ?>" class="regular-text" placeholder="ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" autocomplete="off"></td>
						</tr>
						<tr>
		...
		<aside class="pmp2fa-sidebar">
			...
			<div class="pmp2fa-box">
				...
				<p><strong><?php esc_html_e( 'Author:', 'pmp-2fa-authentication' ); ?></strong>
					<a href="https://linktr.ee/fahadkhalid211" target="_blank" rel="noopener noreferrer">Fahad Khalid</a>
				</p>
				<p><strong><?php esc_html_e( 'Support:', 'pmp-2fa-authentication' ); ?></strong>
					<a href="https://codecanyon.net/" target="_blank" rel="noopener noreferrer">CodeCanyon</a>
				</p>
			</div>
			...
			<div class="pmp2fa-box">
				...
				<p class="description" style="margin-bottom:10px;">
					<?php esc_html_e( 'Revoke your own trusted devices — OTP verification will be required on your next login.', 'pmp-2fa-authentication' ); ?>
				</p>
				<button type="button" id="pmp2fa-revoke" class="button button-secondary">
					<?php esc_html_e( 'Revoke My Devices', 'pmp-2fa-authentication' ); ?>
				</button>
				<span id="pmp2fa-revoke-result" style="display:block;margin-top:6px;font-size:13px;" aria-live="polite"></span>

				<hr style="margin:16px 0;">

				<p class="description" style="margin-bottom:10px;">
					<strong><?php esc_html_e( 'Revoke for a specific user:', 'pmp-2fa-authentication' ); ?></strong><br>
					<?php esc_html_e( 'Search by username, email address, or user ID.', 'pmp-2fa-authentication' ); ?>
				</p>
				<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
					<input
						type="text"
						id="pmp2fa-revoke-user-input"
						class="regular-text"
						placeholder="<?php esc_attr_e( 'Username, email, or user ID', 'pmp-2fa-authentication' ); ?>"
						style="flex:1;min-width:180px;"
						autocomplete="off"
					>
					<button type="button" id="pmp2fa-revoke-user" class="button button-secondary" disabled>
						<?php esc_html_e( 'Revoke', 'pmp-2fa-authentication' ); ?>
					</button>
				</div>
				<span id="pmp2fa-revoke-user-result" style="display:block;margin-top:6px;font-size:13px;" aria-live="polite"></span>
			</div>

			<div class="pmp2fa-box">
				<h3>💡 <?php esc_html_e( 'Quick Tips', 'pmp-2fa-authentication' ); ?></h3>
				<ul style="list-style:disc;padding-left:18px;font-size:13px;color:#555;line-height:1.9;margin:0;">
					<li><?php esc_html_e( 'Works with PMP frontend shortcode login.', 'pmp-2fa-authentication' ); ?></li>
					<li><?php esc_html_e( 'OTPs are hashed — never stored in plain text.', 'pmp-2fa-authentication' ); ?></li>
					<li><?php esc_html_e( 'SMS falls back to email when Twilio is unavailable.', 'pmp-2fa-authentication' ); ?></li>
					<li><?php esc_html_e( 'For SMS, users must add a phone number in their profile.', 'pmp-2fa-authentication' ); ?></li>
					<li><?php esc_html_e( 'Trusted-device cookies are HttpOnly and hashed.', 'pmp-2fa-authentication' ); ?></li>
				</ul>
			</div>

		</aside>
	</div><!-- /.pmp2fa-admin-layout -->
</div><!-- /.wrap -->
